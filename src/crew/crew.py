...
class CleanMarketplaceAnalysisCrew:
    ...
    def _execute_clean_tasks_manually(self, query: str, query_id: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            # Use Daraz product title if available, otherwise original query
            search_query = query
            if results['task_a_daraz'].get('status') == 'success' and results['task_a_daraz'].get('title'):
                search_query = results['task_a_daraz']['title']
                logger.info(f"🔄 Using Daraz product title for search: {search_query[:50]}...")
